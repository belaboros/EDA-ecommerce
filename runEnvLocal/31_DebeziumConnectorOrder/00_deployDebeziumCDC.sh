...
curl --silent -i -X PUT -H  "Content-Type:application/json" \
    http://${CONNECTHOST}:8083/connectors/order-postgres-outbox-source-connector/config \
    -d '{
            "connector.class": "io.debezium.connector.postgresql.PostgresConnector",
            "plugin.name": "pgoutput",
            "database.hostname": "postgresOrder",
            "database.port": "5432",
            ...
            "transforms.outbox.route.topic.replacement": "MSfFP.OrderMgmt.event.${routedByValue}", 
